...
| `averaging_down` | 3 | entrée | re-prise même symbole+sens dont les 2 dernières sur ce setup ont perdu (trades intercalés ignorés, un gain reset, taille indifférente) |
...
| `unfamiliar_symbol` | 1 | entrée | symbole hors des instruments habituels (fenêtre 15 j, min 8 trades d'historique) |
...
| `max` | Max | 29€/mois | 18 détecteurs, mode prop firm, débriefs IA (jour/semaine/mois), rapport hebdo, règles configurables, alertes Telegram |
...
- Early adopters −25 % à vie (Pro 14,25€ / Max 21,75€), **limité aux 25 premiers inscrits** : coupon Stripe `caldra-early-25-forever` (`duration: forever`, percent_off 25, max_redemptions 25) + code promo START25. Le champ "code promo" s'affiche au checkout.
...
1. Créer deux produits (Pro 19€/mois, Max 29€/mois) → copier les Price IDs dans `.env.local` et Vercel
...
- **PnL neutre (`#e2e8f0`) sur l'écran Session (live)** — anti-biais cognitif pendant le trading. **Analytics (revue rétrospective) : vert/rouge CIBLÉ** — réservé aux **valeurs P&L chiffrées** (P&L net, gain attendu, € par symbole, meilleur/pire trade, Long/Short, label par jour) et à la **ligne de la courbe d'equity**. Les barres, le donut win rate et les jauges restent **neutres** (la direction/magnitude porte déjà le signe) — anti-saturation, depuis 2026-06-21. SVG : pas de texte d'axe en `<text>` avec `preserveAspectRatio="none"` (ça s'étire) → libellés en HTML superposé ; traits de courbe en `vectorEffect="non-scaling-stroke"`.
...
### ✅ Stripe configuré
- Produits : Pro 19€/mois (`STRIPE_PRO_PRICE_ID`) + Max 29€/mois (`STRIPE_MAX_PRICE_ID`)
