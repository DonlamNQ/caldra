...
function PnlChart({ series }: { series: DayStats[] }) {
    ...
    return (
      <div>
        <svg viewBox={`0 0 ${W} ${H}`} style={{ width: '100%', height: 150 }}>
          ...
          <text x={W / 2} y={yMid + 4} textAnchor="middle" fill="rgba(226,232,240,.3)" fontSize="10" style={{ fontFamily: 'var(--font-geist-mono),monospace' }}>// aucune donnée</text>
        </svg>
      </div>
    )
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 9, color: 'rgba(226,224,218,.2)', fontFamily: "var(--font-geist-mono),monospace", marginTop: 4 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6,1fr)', gap: '.875rem', marginBottom: '1.25rem' }}>
            {[
              ...
              <div key={i} style={{ ...CARD, padding: '1rem 1.1rem' }}>
                ...
                <div style={{ fontSize: 'clamp(1rem,1.6vw,1.3rem)', fontWeight: 500, color: s.color, fontVariantNumeric: 'tabular-nums', fontFamily: "var(--font-geist-mono),monospace", letterSpacing: -.5 }}>{s.value}</div>
              </div>
            ))}
          </div>
                  ...
                  {freq.map(({ type, count, level }) => (
                    <div key={type}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 5 }}>
                        <span style={{ color: 'rgba(226,224,218,.4)', fontSize: 10, fontFamily: "var(--font-geist-mono),monospace", textTransform: 'uppercase', letterSpacing: .5 }}>{type.replace(/_/g, ' ')}</span>
                        <span style={{ color: DOT_COLORS[level] ?? 'rgba(226,224,218,.4)', fontSize: 10, fontWeight: 600, fontFamily: "var(--font-geist-mono),monospace" }}>{count}</span>
                      </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                  ...
                  <tbody>
                    {[...series].reverse().map(s => (
                      <tr key={s.date}>
                        <td style={{ padding: '7px 8px', color: 'rgba(226,224,218,.4)', fontFamily: "var(--font-geist-mono),monospace", borderBottom: '0.5px solid rgba(255,255,255,.04)' }}>{s.date}</td>
                        ...
                        <td style={{ padding: '7px 8px', color: '#e2e8f0', fontFamily: "var(--font-geist-mono),monospace", fontWeight: 600, borderBottom: '0.5px solid rgba(255,255,255,.04)', fontVariantNumeric: 'tabular-nums' }}>
