...
- [~] **#8 Autres plateformes futures** — recherche faite : seules les voies « gratuit + sans bot » sont retenues. **IBKR (Flex Web Service, modèle MT5)** + **TradeStation (OAuth, modèle cTrader)** CODÉS (2026-06-29). **Tradovate** = via accord vendeur plus tard. Rithmic/ProjectX = payant/serveur interdit (écartés). NinjaTrader/Quantower/MotiveWave = add-on chez l'user (écartés). **Code 100 % fait** (2026-06-29) : migrations v2.20 + v2.21 **exécutées en prod** ✅ ; UI Intégrations câblée + réordonnée (MT5 → cTrader → IBKR, puis « Prochainement ») ✅ ; workers `ibkr-worker.js` + `tradestation-worker.js` complets + guide `worker/SETUP-RAILWAY.md` ✅. **IBKR : worker déployé sur Railway et EN LIGNE** (2026-06-29, `MT5_ENC_KEY` confirmée identique à Vercel par test de déchiffrement) ✅ — reste à valider avec un vrai compte IBKR. **TradeStation : MIS DE CÔTÉ jusqu'après le lancement** (peu d'apport au lancement + l'API ne renvoie pas le P&L réalisé par ordre → seuls les détecteurs d'entrée marcheraient). Carte passée en « Prochainement », plomberie OAuth/worker conservée. Réactivation post-lancement = enregistrer l'app TradeStation + redéployer la carte + service Railway (`railway.tradestation.json`).
