...
- [~] **#8 Autres plateformes futures** — recherche faite : seules les voies « gratuit + sans bot » sont retenues. **IBKR (Flex Web Service, modèle MT5)** + **TradeStation (OAuth, modèle cTrader)** CODÉS (2026-06-29). **Tradovate** = via accord vendeur plus tard. Rithmic/ProjectX = payant/serveur interdit (écartés). NinjaTrader/Quantower/MotiveWave = add-on chez l'user (écartés). **Code 100 % fait** (2026-06-29) : migrations v2.20 + v2.21 **exécutées en prod** ✅ ; UI Intégrations câblée (page `/connect/ibkr`, cartes dashboard + statut live + déconnexion, OAuth TradeStation) ✅ ; workers `ibkr-worker.js` + `tradestation-worker.js` complets, scripts npm `start:ibkr`/`start:tradestation` + `worker/.env.example` documenté ✅ ; guide `worker/SETUP-RAILWAY.md` ✅. **RESTE (opérationnel)** : déployer les 2 workers Node sur Railway (à côté de cTrader, pas besoin du VPS) ; enregistrer l'app TradeStation (`TRADESTATION_CLIENT_ID/SECRET` sur Vercel + Railway) ; tester chacun avec un vrai compte.
